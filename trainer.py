...
# ------------------------------
# PARAMETERS (FAST + STABLE)
# ------------------------------
BATCH_SIZE = 8           # keep small to avoid kaggle hang
EPOCHS = 1               # Stage-A demo (change later to 5/10/20)
LEARNING_RATE = 1e-4
SEED = 42
...
# On-the-fly cheap degradation (no extra libs)
# ------------------------------
def degrade_waveform(waveforms):
    """
    waveforms: [B, 1, T]
    returns degraded version (cheap codec-ish effect) same shape
    """
    x = F.avg_pool1d(waveforms, kernel_size=4, stride=4)      # [B,1,T/4]
    x = x.repeat_interleave(4, dim=-1)                        # [B,1,~T]
...
def make_quality_labels(batch_size, p_degrade=0.5, device="cpu"):
    """
    0 = clean, 1 = degraded
    """
...
def load_dataset():
    ...
    raise ValueError("This trainer.py (fast version) is for single-domain training only (CROSS_DOMAIN=False).")
...
def train():
    ...
    if dataset_size == 0:
        raise ValueError("Dataset is empty. Check preprocessing paths.")
    ...
    # num_workers=0 to avoid kaggle hang
    ...
    for epoch in range(EPOCHS):
        ...
        for waveforms, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS} - Training"):
            ...
            # ---- create quality labels + degrade half batch ----
            ...
            # ---- forward ----
            cm_logits, q_logits = model(waveforms_used)          # [B,1], [B,2]
            cm_prob = torch.sigmoid(cm_logits).squeeze()         # [B]

            # ---- losses ----
            loss_cm = criterion(cm_prob, labels)
